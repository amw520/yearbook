...
function createContion(orginData){
	...
	for(var i=0,j=orginData[type];i<j.length;i++){
		...
		if(i<4){//默认选中
			if(1){//坐标
				...
				chartData["categories"][i]["label"]=j[i].value;//取值
			} else {//分类
				chartData["dataset"][i]={};
	...
	for(var k=0;k<schtp.length;k++){
		str="";
		for(var m=0,n=schtp[k];m<orginData[n].length;m++){//数据与坐标对应
...
function createData(orginData){
	...
	chartData.chart.legendShadow=($(".hidechart:checked").size())>0?0:1;
